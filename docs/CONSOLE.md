...
Hash routes (`parseHash`): `#/` Fleet; `#/runs/{id}` Activity; `#/runs/{id}/{section}` with section in `activity | changes | artifacts | usage | evidence | log` (unknown falls back to Activity); `#/attention`; `#/attention/messages`; `#/config` (the last-used Config tab); `#/config/{tab}` with tab in `profiles | groups | identities | storage | audit | settings | logs | diagnostics` (unknown falls back to the last-used tab); `#/pair/{code}` (a scanned pairing link: while the browser is unpaired the code is copied into the pairing form, then the route continues to `#/config`). Anything else is Fleet. Opening a different run resets `store.detail`, seeds the header from the fleet row when known, and polls at once.
...
A waiting run with kind attention, permission, or verification adds an "Answer →" link. Stop takes an optional reason in the confirm. "Stop tree" appears next to Stop only when the detail payload reports `child_count > 0`; it uses the same confirm and reason input and posts `POST /api/runs/{id}/stop-tree` (`// --- stop-tree ---` block). Pause and Resume post with an empty body; `409` on Resume shows "Run is no longer waiting". Reroute fills routes from `/api/models` (5 min cache, "Retry catalog" on failure) and posts provider, model, effort, message. Retry and Continue call `spawnLineage()`, which keeps one `request_id` per run and kind until the POST succeeds, then routes to the new run. Merge is a danger confirm; the result or the refusal becomes a block at the top of Changes.

- *Activity*: a brief strip with the objective, the merged thread of events and messages (`mergeThread`), and a handoff block with the summary or error once terminal. `threadEntryKind` classifies rows from the real DSH journal shapes: `dsh.assistant/message` (parts typed `reasoning`/`text`/`tool-call`) renders as an assistant row with a 💭 thinking block and the message text; `dsh.user/message` is a 🧑 prompt row; `acp.tool_call` is a ⚙ tool row. Machine rows — quiet lifecycle, `dsh.goal/change`, the hundreds of `dsh.assistant/chunk` stream fragments, `acp.agent_thought_chunk` (a duplicate of the reasoning), `dsh.tool/*`, `dsh.step/*`, usage and session bookkeeping — stay hidden until "Show machine events". `acp.tool_call_update` rows patch their tool row by `toolCallId`. Follow-live tracks the scroll position (72 px tail threshold); scrolling up unchecks it and shows "Return to live". The Direct modal has two submit buttons: "Tell at boundary" (`tell`) and "Interrupt now" (`interrupt`). `delegation.antigravity` events (the `orchestra-next delegate` bridge, see docs/ANTIGRAVITY_SPIKE.md) render as a 🛰 row with the objective excerpt, per-turn and conversation token counts from `delegationSummary()`, and the bounded response open when short; failures show the error in the bad colour. Evidence lists the run's delegations from `GET /api/runs/{id}/delegations`, Usage marks `provider: antigravity` rows as a separate subscription, the run header shows an "antigravity allowed" chip, and the dispatch form's Advanced section carries the `allow_antigravity` checkbox.
...
- *Log*: the tail of the run's raw ACP log (`GET /api/runs/{id}/log`, `<state>/runs/{id}/acp.jsonl`) in a `<pre>`, with Follow, a 64 KiB / 256 KiB tail selector, and Refresh. The `logs` block owns both tails (`LOGS.run`, `LOGS.service`): `fetchLog` asks for `?after=<size>` and appends; a reply whose `offset` differs from the known end replaces the text. `syncLog` runs on every render of the panel, on `hashchange` (deferred past the router), and on `visibilitychange`; it keeps a 3 s timer only while the section is visible and the run is active, and fetches once for a terminal run.
...
The view has two sub-tabs, Attention and Messages (`#/attention/messages`). Messages is the fleet-wide ledger from `GET /api/messages` (`outbox` block): a table with status chip (queued, delivered, undeliverable), kind glyph, run link, body excerpt, sent and delivered ages; selects filter by status and kind; "Load older messages" pages back on the `before` cursor. The page reloads once per board revision. A chip beside the Messages sub-tab counts undeliverable messages; the nav badge still counts open attention only.

**Config.** Tabbed; one tab renders at a time, so a tab's data loads only when it is shown (`CONFIG_RENDERERS` in the `config-tabs` block maps tab → render function). Profiles (table; New/Edit dialog with datalists from `/api/models`, manual entry when the catalog fails; Enable/Disable and Archive via revision-checked PATCH; a `409` keeps the operator's edits and reloads the untouched fields). Groups (table; New dialog; Rename and Set directory through the confirm input; Archive confirm). Identities: devices and service tokens (revoke with confirm; new token shown once in `token-reveal` and cleared on close) plus Pairing (`POST /api/auth/pair` → code, expiry countdown, and a QR of `${origin}/#/pair/{code}` drawn from the pure `qrMatrix()` in the `qr` block). Storage (`/api/storage` report; "Plan prune" posts a dry-run plan whose exact items and totals render as a table; "Apply this plan" is a danger confirm restating the count and size, then shows what moved and the trash directory). Audit (controls and callbacks feeds loaded lazily with an `after` cursor, a client-side text filter, and "Load more"). Settings is an empty shell ("Coming soon") between the `config-tab:settings` anchors in `index.html` and `app.js`. Logs (`config-tab:logs`) tails the launchd service log through `GET /api/service-log` with the same Follow / tail size / Refresh controls as the run Log section, a 3 s timer that runs only while the tab is visible, and a Download link to `GET /api/service-log/raw`; a missing file shows the API's `note`. Settings also carries a *Version* block (`self-update` block): the running branch and commit, "Check for updates" (`POST /api/update/check`) listing the commits behind `origin/main`, and "Update and restart" (`POST /api/update/apply`, danger confirm) when the checkout is clean and only behind; the daemon restarts itself and the console reconnects on the next poll. Diagnostics: identity, instance, board revision, poll health, and `/api/readiness` (schema, DSH, Claude sidecar, models cached).
...
| `1`–`6` | Run | Activity, Changes, Artifacts, Usage, Evidence, Log |
...
| `tests/test_ui_document.py` | Static conformance: files present; the eight Config tab panels and anchors, `#/config/{tab}` parsing; the Logs tab controls and the run `log` section; `node --check` on `app.js`; no HTML injection; no inline script, style, or handlers; sentinel pairing and required names; every `data-action`/`data-form` has a handler; dark scheme, reduced motion, `scrollbar-gutter`, `color-scheme` meta. |
...
2. Put DOM and store code in the feature block: render functions, `Object.assign(ACTIONS, {...})`, `Object.assign(FORMS, {...})`, and any listeners on static elements. A new Config area is a tab: add its name to `CONFIG_TABS`, a `<section id="config-{tab}" data-ctab="{tab}" role="tabpanel" hidden>` between `<!-- config-tab:{tab} -->` anchors in `index.html`, and its `renderConfig*` function in `CONFIG_RENDERERS` between `// config-tab:{tab}` anchors. Logs still has its shell; Settings is filled.
