...
function uiStateEncode(state) {
  ...
  return JSON.stringify({ filters, machine: Boolean(state.ui.machine), section: state.ui.section, configTab: state.ui.configTab, usageWindow: state.ui.usageWindow, dispatch });
}
function uiStateDecode(raw) {
  const out = { filters: { status: new Set(), group: "", profile: "", text: "" }, machine: false, section: "activity", configTab: "profiles", usageWindow: "24h", dispatch: { profile: "", group: "" } };
  try {
